...
\title{LACE 2: an interactive R tool for the inference and visualization of longitudinal cancer evolution models}
...
LACE 2 is a new release of the LACE R package. LACE 2 is capable of performing 
clonal evolution analyses for single-cell sequencing data including longitudinal 
experiments. LACE 2 allows to annotate variants and retrieve the relevant mutations 
...
using boolean matrix factorization. Furthermore, LACE 2 allows to investigate 
...
\section{Installation of LACE 2 R
package}\label{installation-of-lace-2.0-r-package}

The package is available on GitHub and Bioconductor. LACE 2 requires R
...
To install LACE 2 run:

\begin{verb}
remotes::install_github("https://github.com/BIMIB-DISCo/LACE", dependencies = TRUE)
\end{verb}

LACE 2 uses \emph{Annovar} and \emph{Samtools suite} as back-ends for
...
\section{Running LACE 2}\label{run-lace-2.0}

To start LACE 2 user interface run:
...
\section{Using LACE 2}\label{using-lace-2.0}

LACE 2 has been thought to be used on single cell sequencing data for
...
derivation are computationally expensive steps. LACE 2 reduces
...
LACE 2 automatically recognizes if the selected path is a LACE project
...
Under Projects, there is the sub-menu Demos. LACE 2 provides the user
with two demos:
...
data as part of the library preparation. LACE 2 accepts tsv/csv
