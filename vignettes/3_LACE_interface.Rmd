...
LACE 2.0 is a new release of the LACE R package. LACE 2.0 is capable of performing clonal evolution analyses for single cell sequencing libraries of longitudinal experiments using variant calling data. LACE 2.0 allows to annotate variants and retrieve the relevant mutations interactively based on user filter criteria; it infers the most likelihood clonal tree, cell matrix attachment and false positive/negative rates using a Bayesian model and boolean matrix factorization. Furthermore, it generates the longitudinal clonal tree. Finally, LACE 2.0 allows to investigate the longitudinal clonal expansion under different experimental conditions and the occurrence of single mutations which can be queried via *ensembl* database.   
...
LACE 2.0 requires R > 3.6.3 and was tested on Windows, MacOS and Linux.

To install it, Bioconductor needs to be installed. To do so run:
```{r, echo = T, eval = FALSE}
...
remotes::install_github(
    "https://github.com/BIMIB-DISCo/LACEinterface",  
    dependencies = TRUE)
```

All the other R dependencies will be automatically downloaded and installed.

LACE 2.0 uses *Annovar* and *Samtools suite* as back-ends for variant call annotation and depth computation, respectively. Please refer to the next section to intsall them.

## Installation of other softwares required by LACE 2.0 
...
LACE 2.0 has been thought to be used on standard single cell sequencing data for which it is available variant calling data in standard VCF format and binary aligned data in standard BAM format. 
...
The operation is followed by the possibility from the user to select variants which are drivers of the understudied biological problem. 
 ...
 * Melanoma dataset [cite Rambow et Al.] 
...
Hence, the user can press "Select variants" (![](select_variant.png){width=10%}) to perform the aforementioned computations on VCF and BAM files to derive all the necessary aggregated information on the sampled cells. Afterward, the user is presented with a interactive live preview of variants passing the filters (including relevant parameters which can help in the selection) while values are changed. 
