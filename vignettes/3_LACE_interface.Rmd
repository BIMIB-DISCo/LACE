...
LACE 2 is a new release of the LACE R package. LACE 2 is capable of performing clonal evolution analyses for single-cell sequencing data including longitudinal experiments. LACE 2 allows to annotate variants and retrieve the relevant mutations interactively based on user-defined filtering criteria; it infers the maximum likelihood clonal tree, cell matrix attachments and false positive/negative rates using boolean matrix factorization. Furthermore, LACE 2 allows to investigate cancer clonal evolution under different experimental conditions and the occurrence of single mutations which can be queried via *ensembl* database.

## Installation of LACE 2 R package

The package is available on GitHub and Bioconductor.
LACE 2 requires R > 4.1.0 and Bioconductor.
...
LACE 2 uses *Annovar* and *Samtools suite* as back-ends for variant calling annotation and depth computation, respectively. Please refer to the next section to install them. 
...
If the scripts are installed in binary search path, then LACE 2 will detect them automatically. 
...
LACE 2 has been thought to be used on single cell sequencing data for which it is available variant calling data in standard VCF format and binary aligned data in standard BAM format. 

The user is provided with an interface to initiate a project and to set filter thresholds after which annotation of variants, filtering of data and depth at variant sites are retrieved. Both annotation and depth derivation are computationally expensive steps. LACE 2 reduces possible re-computation by detecting parameter variations of the user interface and by comparison of the timestamps of interface state, inputs and outputs. Intermediary and final data are stored in the designated folders. 
...
LACE 2 automatically recognizes if the selected path is a LACE project and proposes the user to load it 
...
LACE 2 provides the user with two demos:
