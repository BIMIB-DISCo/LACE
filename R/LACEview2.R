...
lace_interface<-function (B_mat, clones_prevalence, C_mat, error_rates, width = NULL, height = NULL,
                elementId = NULL, info='')
{
    ...
    for (i in set_of_roots) {
      ...
      for (j in tmp_set_of_roots) {
        ...
        if (sum(B[i,])==proj) {
          ...
          print(paste(i,'is parent of', j))
          #print(set_of_roots)
        }
        if (sum(B[j,])==proj) {
          set_of_roots=setdiff(set_of_roots,i)
          print(paste(j,'is parent of', i))
          #print(set_of_roots)
        }
      }
    }
  ...
  forest_check <- function(B) {
    ...
    for(i in 1:nrow(B)) {
      ...
      for(j in i:nrow(B)) {
        ...
        print(paste(rownames(B)[i], rownames(B)[j], check_forest[j,1]))
      }
    ...
    print(check_forest)
    print(check_forest_control)
    print(all(check_forest==check_forest_control))
    is_forest=all(check_forest==check_forest_control)
    if (is_forest) {
      print("B represents a forest")
    } else {
      print("B is not a forest or a tree")
    }
    return(is_forest)
  }
  ...
  add_unique_root <- function(B, root_name='Root') {
    ...
    if (num_roots==1) {
      print("nodes in B are sprouting from a single node")
    }
    else {
      print("there are multiple roots")
    }

    if(B_rank==nrow(B) || B_rank==ncol(B)) {
      print('B is a full-rank matrix.')
      if(forest_check(B)) {
        print('B has a forest like structure')

        if (nrow(B)>ncol(B)) {
          print('B is not a square matrix. B has not enough mutations.')
          ...
          print('B is not a square matrix. B has not enough clones.')
          if (nrow(B)+1==ncol(B)) {
    ...
    if (!(B_rank==ncol(B) && B_rank==nrow(B))) {
      print('Adding a root does not resolve the problem, B is till not a full rank matrix')
    }
  ...
  print(adj_to_B(adj_matrix, clone_labels))
  ...
  chrono_occurences <- function(adj_matrix, prevalence) {
    ...
    for(target in 1: length(rownames(prevalence))) {
      ...
      if (times[target] < times[source]) {
        print(times[target],times[source])
    ...
    if (all(times==times_pre)) {
      print('No incongruences found due to the chronological order')

      print('first time mutation occurrences based the chronological order of samples:')
      print(times)
      print('adjacent matrix with first time occurrences:')
      print(adj_matrix_t)

    } else {
      print('The longitudinal clonal tree has been corrected')

      print('pre-computation of first time mutation occurrences based only on prevalence values different from zero:')
      print(times_pre)
      print('adjacent matrix with pre-computed first time occurrences:')
      print(adj_matrix_t_pre)
  ...
  for (t_1 in times_n[(length(times_n)):1]) { # instead of 2 as smaller point
    ...
    for (source in 1:nrow(adj_matrix)) {
      ...
      if (t_1==times_n[(length(times_n))]  && prevalence[target,t_1]==0 ){ #&& prevalence[source,t_1-1]==0) {
        ...
        print(c(source_t1, target_t1, source_t1_prev, target_t1, value))
        ...
        #target mut_time+1
        value=0 #for dead breaches
        print(c('removed', target_t1))
        print(c(source_t1, target_t1, source_t1, target_t1_next, value))
