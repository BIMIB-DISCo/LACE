...
/* function to download PNG
	...
	*/
	function _downloadPNG_(className, fileOutputName) {
	  var canvasElement = document.getElementById("container");
	  console.log(canvasElement);
	  var MIME_TYPE = "image/png";
    var imgURL = canvasElement.toDataURL(MIME_TYPE);
    var dlLink = document.createElement('a');
    dlLink.download = fileOutputName;
    dlLink.href = imgURL;
...
function _run_timescape(view_id, width, height, userConfig) {
  view_id='streamgraph';
	...
	var canvasDIV = d3.select("#" + view_id).append("div")
	    ...
	    .style("width", width + "px")
	    .attr("class", "div")
	    .attr("id", view_id+"_canvas")
	    .style("float", "left")
	    .style("transform", "rotate(90deg) translate("+dim.canvasSVGHeight/2+"px,"+0+"px)");
	...
	*/
	...
	var canvasSVG = canvasDIV
	    ...
	    //.attr("transform", "rotate(90) translate("+dim.canvasSVGHeight/2+" "+0+")")
	    .style("float", "left");
	...
	curVizObj.view.tsSVG
	    ...
	    .on('mouseover', function(d,i) {
            ...
            Object.keys(tip).forEach(function (e){
              tip[e].destroy();
            });

            clonal_nodes.forEach(function( ele ){
            	tip[ele.id()] = makeTippy(ele,ele.data()["prev"]);
            	tip[ele.id()].show();
            	//setTimeout(function(){  tip[ele.id()].destroy(); }, 4000);
            });
            //if(edge.data()["name"] in mutdict) {
              clone.predecessors().forEach(function( ele ){
                //ele.style('opacity','1');
                if(ele.isEdge()){
                  if(ele.data()["name"] !== ""){
	    ...
	    //#### fishplot
	        ...
	        if (e.data().target.indexOf(d.gtype) !== -1) {
	          if(Boolean(cy.mutdict)){
  	          if(e.data()["name"] in cy.mutdict){
                  found=true;
                  var tablecontainer =  document.getElementById("lacetable")
                  tablecontainer.style.visibility = 'visible'
	...
	curVizObj.view.xAxisSVG
	    ...
	    .attr('y', 5)
	    //.attr('y', -dim.tsSVGHeight - dim.xAxisHeight/2)
	    ...
	    .on('mouseover', function(d, i) {
	        if (!dim.selectOn && _checkForCellScapeSelections(view_id)) {
              ...
              cy.$('#T'+i).style('opacity','1');
              cy.$('#T'+i).children().style('opacity','1');
              cy.$('#T'+i).children().connectedEdges().style('opacity','1');

              Object.keys(tip).forEach(function (e){
                tip[e].destroy();
              });

              cy.$('#T'+i).children().forEach(function( ele ){
	...
	var node = curVizObj.view.tsTree.selectAll(".legendTreeNode")
	    ...
	    .on('mouseover', function(d) {
	        ...
	        //tip[clone_name] = makeTippy(d.id,clone_name); //error
	        //tip[clone_name] = tippy('#'+d.id, {content: 'Tooltip',});
          //console.log()
          //console.log(document.getElementById(d.id))
          //console.log(document.getElementById(d.id).className)
          ...
          tip[clone_name] = new tippy(this, {content: "Clone: " + clone_name,  showOnCreate: true, placement: 'bottom',});
	    ...
	    .on("click", function(d) {
	        ...
	        var found=false;
	        cy.edges().forEach(function(e){
	        if (e.data().target.indexOf(d.id) !== -1) {
	          if(Boolean(cy.mutdict)){
  	          if(e.data()["name"] in cy.mutdict){
                  found=true;
                  var tablecontainer =  document.getElementById("lacetable")
          ...
          tablecontainer.style.visibility = 'hidden'
	        ...
	        if (curVizObj.userConfig.mutations_provided && !dim.mutSelectOn) {
	            ...
	            if (dim.nClickedNodes == 1) {
	                ...
	                // _makeMutationTable(curVizObj, curVizObj.view.mutationTableDIV, filtered_muts,dim.mutationTableHeight);
	...
	if (curVizObj.userConfig.mutations != "NA") {
	    ...
	    // make the table
	    // _makeMutationTable(curVizObj, curVizObj.view.mutationTableDIV, curVizObj.data.mutations, dim.mutationTableHeight);
	}
	...
	*/
	function _createLabels(curVizObj, label_data) {
        ...
        curVizObj.view.tsSVG
            ...
            .attr('y', function(d) {
              var y=dim.tsSVGHeight - (1 - d.middle)*dim.tsSVGHeight
              if(isNaN(y)) {
                  y=0;
                }
              return y;
	...
	*/
	function _getCPData(curVizObj) {
	    ...
	    //curVizObj.data.cp_data = cp_data_original;
	}
...
//					var cp_data_norm = {};
//					var tp_total = {};
//					Object.keys(curVizObj.data.cp_data_original).forEach(function(tp_key) {
//						// get total cp for this timepoint
//						tp_total[tp_key] = 0;
//						Object.keys(curVizObj.data.cp_data_original[tp_key]).forEach(function(gtype_key) {
//							tp_total[tp_key] += curVizObj.data.cp_data_original[tp_key][gtype_key];
//						});
//						//curVizObj.data.tp_total = tp_total;
//						//var tp_total_max = $.map(tp_total, function(value, key) { return value }); // find max of tp_total and then normalize
//						// get proportionate cp for each genotype
//						//cp_data_norm[tp_key] = {};
//						//Object.keys(curVizObj.data.cp_data_original[tp_key]).forEach(function(gtype_key) {
//						//	cp_data_norm[tp_key][gtype_key] = curVizObj.data.cp_data_original[tp_key][gtype_key]/tp_total_max;
//						//	tp_total[tp_key] /=tp_total_max;
//						//});
//						//curVizObj.data.tp_total = tp_total;
//						//curVizObj.data.cp_data = cp_data_norm;
//	        	//-------------
//
//	            //_getCentredLayout(curVizObj, curVizObj.data.treeStructure, tp, curVizObj.data.layout, (1-tp_total[tp_key])/2);
//					})
	...
	function _getBezierPaths(paths, tsSVGWidth, tsSVGHeight) {
	    ...
	    $.each(paths, function(path_idx, cur_path) {
	        ...
	        for (var i = 0; i < path.length-1; i++) {
              ...
              var d={};
              d.source={};
              d.source={"y": xsource, "x": ysource};
              //console.log(d)
              //console.log(d.source)
              //console.log(d.source.x)
              d.target={"y": xtarget, "x": ytarget};
              //d.source(function() { return {"y": xsource, "x": ysource }; })
	            // .target(function() { return {"y": xtarget, "x": ytarget};})
              diagonal = function link() {
